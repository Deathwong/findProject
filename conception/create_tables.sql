...
(
   fav_annonces         bigint not null,
   use_id               bigint,
   primary key (fav_annonces)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
alter table annonce add constraint fk_annonce_relation__user foreign key (use_id)
      references user (use_id) on delete restrict on update restrict;

alter table categorie_annonce add constraint fk_categori_relation__annonce foreign key (ann_id)
      references annonce (ann_id) on delete restrict on update restrict;

alter table categorie_annonce add constraint fk_categori_relation__categori foreign key (cat_id)
      references categorie (cat_id) on delete restrict on update restrict;

alter table favoris add constraint fk_favoris_relation__user foreign key (use_id)
      references user (use_id) on delete restrict on update restrict;

alter table message add constraint fk_message_relation__user foreign key (mes_sender_id)
      references user (use_id) on delete restrict on update restrict;

alter table message add constraint fk_message_relation_receiver__user foreign key (use_receiver_id)
      references user (use_id) on delete restrict on update restrict;
